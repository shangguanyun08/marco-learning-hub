...
Four dated follow-up sessions linked prominently from `harry-math-practice/`:
...
Attempts include timestamps and are saved under the isolated localStorage key `harry-star-sept20-four-sessions-v1`. This does not touch existing Q1–Q35 practice storage or its online synchronization. New sessions are local-device records, with JSON export. A completed session can start a new run, appending rather than replacing history. New runs are identified separately and do not retroactively change an earlier score. No student scoring is sent to a new external service.

Run all old and new regression tests with `npm test` in `harry-math-practice/`. Tests cover 48-item coverage, unchanged originals, independently calculated keys, first-try scoring, retries and reveal, reload persistence, separate session scores, completed runs, and retained history. Browser QA additionally covers mobile layout, navigation, and actual radio-button submissions. Test submissions are made only on localhost; published learner scores remain untouched.
